                ...
                {(props.pather.length !== 0) && props.pather.map((paParts, i) =>{
                    ...
                    switch(true){
                        ...
                        case i === props.pather.length - 1:
                            return null;
                        case i < props.pather.length:
                            newBuild = props.pather.slice(0, i + 1).join('/');
                            break;
                        case props.pather[i].includes('type1='):
                            newBuild = props.pather.slice(0, i + 1).join('/');
                    ...
                    return <li key={'patholder' + i}> <Button key={'pathed' + i} className='nav-butt' children={pathNamer && pathNamer} onClick={props.onClick} attribsext={{'data-pathing': (newBuild === '' ? null : newBuild)}} /></li>
                })}
            </ul>      
        </div>
    );
